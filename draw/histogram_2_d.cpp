...
Histogram2D::Histogram2D(const std::string &file_name,
  ...
  for( const auto& name : objects ){
    histograms.push_back( FileManager::ReadObject<TH2>(file_name, name) );
  }
  histo2d_ = std::unique_ptr<TH2>( dynamic_cast<TH2*>(histograms.front()->Clone()) );
...
void Histogram2D::Projection(const std::string& axis, int first_bin, int last_bin) {
  ...
  if( axis == "X" || axis == "x" ){
    histogram_ = std::unique_ptr<TH1>( dynamic_cast<TH1*>(histo2d_->ProjectionX( proj_name.c_str(),first_bin, last_bin )) );
  }
  else if( axis == "Y" || axis == "y" ){
//    auto histo = histo2d_->ProjectionX( proj_name.c_str(),first_bin, last_bin );
    histogram_ = std::unique_ptr<TH1>( dynamic_cast<TH1*>(histo2d_->ProjectionY( proj_name.c_str(),first_bin, last_bin )) );
  }
  else
