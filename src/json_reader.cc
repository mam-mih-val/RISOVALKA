...
Draw::Picture GetPictureConfig(const std::string &json_file) {
  ...
  picture.save_name = config.get<std::string>("save name", "");
  ...
  std::vector legend_position{
      legend_config.get<double>("x1"),legend_config.get<double>("y1"),
      legend_config.get<double>("x2"),legend_config.get<double>("y2"),
  };
  picture.legend_position = legend_position;
  auto axis_config = config.get_child("axis title");
  std::string axis_names = ";"+ axis_config.get<std::string>("X")
      +";"+axis_config.get<std::string>("Y");
  picture.axis_titles = axis_names;
  auto axis_range_conf = config.get_child("y axis range");
  std::vector axes_range{axis_range_conf.get<double>("min"), axis_range_conf.get<double>("max")};
  axis_range_conf = config.get_child("x axis range");
  picture.y_axis_range = axes_range;
  axes_range = {axis_range_conf.get<double>("min"), axis_range_conf.get<double>("max")};
  picture.x_axis_range = axes_range;
  return picture;
};
